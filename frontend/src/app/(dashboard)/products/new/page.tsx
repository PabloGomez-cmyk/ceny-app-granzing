...
function Select({
  ...
  return (
    <div>
      ...
      <select
        {...props}
        className={`h-[42px] w-full rounded-[10px] border px-3 text-[13px] text-[#0f172a] focus:outline-none focus:ring-2 focus:ring-[#0f6e50]/20 ${
          error
            ? "border-red-400 bg-red-50 focus:border-red-400"
            : "border-[#dde4ee] bg-[#f8fafc] focus:border-[#0f6e50]"
        }`}
        ...
        {PRESET_COLORS.map((c) => (
          <button
            ...
            className={`h-7 w-7 rounded-full transition-transform hover:scale-110 ${
              value === c ? "ring-2 ring-offset-1 ring-[#0f6e50]" : ""
            }`}
            style={{ background: c }}
          />
        ))}
      ...
      {preview ? (
        <div className="relative flex items-center gap-3 rounded-[10px] border border-[#dde4ee] bg-[#f8fafc] p-3">
          ...
          <button
            type="button"
            onClick={onClear}
            className="rounded-full p-1 text-[#94a3b8] hover:bg-[#f1f5f9] hover:text-[#475569]"
          >
            <X size={14} />
          </button>
        </div>
        ...
        >
          {isUploading ? (
            <Loader2 size={20} className="animate-spin text-[#0f6e50]" />
          ) : (
            <Icon size={20} className="text-[#94a3b8]" />
          )}
          <span className="text-[12px] font-medium text-[#475569]">
            {isUploading ? "Subiendo..." : "Hacer clic para subir"}
          </span>
  ...
  return (
    <div className="mt-2 flex items-center gap-2 rounded-[8px] border border-[#0f6e50]/30 bg-[#f0faf6] p-2">
      ...
      <button
        type="button"
        onClick={onCancel}
        className="flex h-6 w-6 items-center justify-center rounded-full text-[#94a3b8] hover:text-[#475569]"
      >
        <X size={12} />
      </button>
    </div>
...
export default function NewProductPage() {
  ...
  const { mutateAsync: createProduct, isPending: saving } = useCreateProduct();
  ...
  function toggleGlass(id: string) {
    setGlassIds((prev) => prev.includes(id) ? prev.filter((g) => g !== id) : [...prev, id]);
  }
    ...
    <div className="flex min-h-screen flex-col bg-[#f0f4f8]">
      ...
      <header className="flex items-center justify-between bg-[#0f6e50] px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-[17px] text-white">Nuevo producto</h1>
            <p className="text-[11px] text-white/70">Completá todos los campos obligatorios</p>
          </div>
        </div>
      </header>
        ...
        <div className="rounded-[14px] border border-[#e8ecf2] bg-white p-6">
          ...
          <div className="space-y-5">
                ...
                <div className="rounded-[12px] border border-[#0f6e50]/30 bg-[#f0faf6] p-4 space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Input
                      ...
                      required
                      placeholder="Ej: 3M, LLumar, SunTek"
                      id="new-brand-name"
                    />
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        const inp = document.getElementById("new-brand-name") as HTMLInputElement;
                        if (inp?.value.trim()) handleCreateBrand(inp.value.trim());
                      }}
                    ...
                    <button
                      type="button"
                      onClick={() => { setShowNewBrand(false); setLogoUrl(null); }}
            ...
            <div>
              ...
              {showNewCategory && (
                <InlineCreate
                  placeholder="Nombre de la categoría (ej: Solar Residencial)"
        ...
        <div className="rounded-[14px] border border-[#e8ecf2] bg-white p-6">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              {errors.application_types && (
                <p className="mt-1 text-[11px] text-red-500">{errors.application_types}</p>
              )}
            </div>
            ...
            <div>
              ...
              {glassTypes.length === 0 && !showNewGlassType ? (
                <p className="text-[12px] text-[#94a3b8]">
                  ...
                  <button type="button" onClick={() => setShowNewGlassType(true)} className="text-[#0f6e50] underline">
                    Crear el primero
                  </button>
                </p>
              ) : (
              ...
              {showNewGlassType && (
                <InlineCreate
                  placeholder="Nombre del vidrio (ej: Monolítico, DVH, Templado)"
                  onConfirm={handleCreateGlassType}
                  onCancel={() => setShowNewGlassType(false)}
                  isLoading={creatingGlassType}
                />
              )}
            </div>
          </div>
        ...
        <div className="rounded-[14px] border border-[#e8ecf2] bg-white p-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <Input
              ...
              hint="Rechazo de rayos ultravioleta"
            />
            <Input
              label="IRR%"
              required
              ...
              hint="Rechazo de calor infrarrojo"
            />
            <Input
              label="TSER%"
              required
              ...
              placeholder="0 — 100"
              value={tser}
              onChange={(e) => setTser(e.target.value)}
              hint="Rechazo total de energía solar"
            />
            <Input
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <Input
              label="Ancho del rollo (cm)"
              type="number"
              min="1"
              step="0.5"
              placeholder="152"
              value={rollWidthCm}
              onChange={(e) => setRollWidthCm(e.target.value)}
              hint="Ancho estándar de la lámina en rollo"
            />
            <Input
              label="Largo del rollo (m)"
              type="number"
              min="1"
              step="0.5"
              placeholder="30"
        ...
        <div className="rounded-[14px] border border-[#e8ecf2] bg-white p-6">
          ...
          <div className="max-w-md">
            <FileUploadZone
              ...
              icon={FileText}
              preview={techSheetUrl}
              isUploading={uploadingSheet}
              onSelect={handleSheetUpload}
              onClear={() => setTechSheetUrl(null)}
            />
          </div>
        </div>
